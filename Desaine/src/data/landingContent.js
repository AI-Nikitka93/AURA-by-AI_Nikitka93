...
Object.assign(copy.ru, {
  document: {
    ...
    description:
      'AURA by AI_Nikitka93 - концептуальный портфолио-проект: бренд премиальных умных украшений, редакционные UX-тексты, интерфейс в эстетике Liquid Glass и фронтенд-реализация.',
  },
  navbar: {
  ...
  heroContent: {
    ...
    disclaimer:
      'AURA - это портфолио-концепт от AI_Nikitka93: бренд-направление, UX-тексты, визуальная система и фронтенд-реализация. Продуктовые сценарии и офферы на странице демонстрационные и не являются реальной витриной магазина.',
  ...
  benefitCards: [
    ...
    },
    {
      ...
      body: 'Каждое изделие создаётся как авторский объект, а не как серийный аксессуар. Это усиливает чувство обладания и делает AURA частью вашего публичного образа.',
      icon: Orbit,
...
Object.assign(copy.ru, {
  ...
  faqSection: {
    ...
    description:
      'Тексты приведены к премиальному тону и не обещают того, что ещё не подтверждено реальными данными или юридическими документами.',
  },
  faqItems: [
    {
    ...
    },
    {
      ...
      answer:
        'На лендинге заявлены только безопасные формулировки. Конкретные обещания о хранении и передаче данных стоит публиковать только после юридически утверждённой политики приватности.',
    },
  ],
  ctaContent: {
    title: 'Смотрите проект, а не только концепт',
    subtitle:
      'AURA показывает, как можно собрать бренд-направление, редакционные UX-тексты, интерфейс Liquid Glass и фронтенд-реализацию в единый портфолио-кейс.',
  ...
  disclaimerContent: {
    ...
    validationText:
      'Эта секция показывает, как мог бы выглядеть модуль валидации и социального доказательства. Метрики, отзывы и коммерческие сигналы не относятся к реальному запуску и используются только как часть портфолио-концепта.',
    footer:
      'Это концептуальная портфолио-страница. Проект демонстрирует бренд-мышление, продуктовый сторителлинг и фронтенд-мастерство. Продукт, офферы, отзывы и сценарии на странице не являются реальной витриной.',
  },
  caseStudyContent: {
    eyebrow: 'Кейс',
    title: 'Что именно показывает этот проект',
    description:
      'AURA - не просто визуальный концепт, а собранный портфолио-кейс, где бренд-стратегия, UX-тексты, интерфейсная система и фронтенд-деплой сведены в один демонстрационный продукт.',
    cards: [
      {
        title: 'Бренд-направление',
        body: 'Позиционирование, тон коммуникации, тёмная палитра и токены Liquid Glass собраны в цельную систему, чтобы концепт выглядел как премиальный fashion-tech бренд.',
      },
      {
        title: 'Система UX-текстов',
        body: 'Первый экран, преимущества, история создателя, CTA и FAQ написаны как редакционный конверсионный слой без фейковых коммерческих обещаний и неподтверждённых заявлений.',
      },
      {
        title: 'Фронтенд-сборка',
        body: 'Лендинг разложен на React-компоненты, собран на Vite + Tailwind и опубликован как рабочий сайт через GitHub Pages и резервный Cloudflare-сценарий.',
      },
      {
        title: 'Подача как кейса',
        body: 'Проект честно подан как концепт-кейс: он демонстрирует процесс, вкус, упаковку и инженерную реализацию, а не притворяется реальным коммерческим бизнесом.',
      },
    ],
    links: [
...
Object.assign(copy.ru, {
  advisor: {
    ...
    description:
      'Этот блок добавляет сайту полноценный продуктовый сценарий выбора: как AURA будет носиться, с какой системой жить и какой сигнал стоит активировать по умолчанию.',
    selected: 'Выбрано',
    steps: {
    ...
    wearMomentOptions: [
      { id: 'daily', label: 'Ежедневный фокус', description: 'Для регулярного ношения, рабочих встреч и спокойного присутствия.' },
      { id: 'evening', label: 'Вечерний сигнал', description: 'Для вечерних выходов, событий и более заметного светового жеста.' },
      { id: 'travel', label: 'Пульс в пути', description: 'Для перелётов, интенсивных дней и ритма, который должен ощущаться быстрее.' },
    ],
    ecosystemOptions: [
      { id: 'ios', label: 'Сначала iPhone', description: 'Приоритет на аккуратный iOS-компаньон, напоминания и настройки с акцентом на приватность.' },
      { id: 'android', label: 'Сначала Android', description: 'Гибкий Android-сценарий, виджеты и больше пользовательского управления поверхностью.' },
    ...
    wearMomentRecommendationMap: {
      daily: { ritual: 'calm', intensity: 42, fitNote: 'Лучше держать посадку ближе к комфортной: указательный или средний палец, спокойный дневной цикл ношения.' },
      ...
      travel: { ritual: 'pulse', intensity: 64, fitNote: 'Нужна более собранная посадка и устойчивость к смене контекста: плотная, но не жёсткая посадка.' },
    },
    ecosystemMap: {
    ...
    output: {
      ...
      idle: 'После применения ритуал и интенсивность автоматически попадут в конфигуратор и форму списка ожидания.',
    },
  },
  ritualConfigurator: {
    ...
    shareStatus: {
      ...
      error: 'Не получилось автоматически открыть или скопировать ссылку, но ручной вариант всё ещё доступен.',
      idle: 'Выбранный ритуал и интенсивность сохраняются и могут быть отправлены вместе с заявкой.',
    },
    advisorButton: 'Сгенерировать персональный результат',
    advisorHint: 'После выбора ритуала можно сразу запустить персональную генерацию. Если удалённый AI недоступен, сайт соберёт локальный результат сам.',
    rituals: [
      {
        id: 'glow',
        name: 'Свечение',
  ...
  emailSignup: {
    ...
    privacyConsent: 'Я прочитал уведомление о приватности и понимаю, как AURA хранит согласие, персонализацию и очередь заявок.',
    ...
    errors: {
      ...
      missingPrivacy: 'Подтвердите уведомление о приватности перед отправкой заявки.',
    ...
    feedback: {
      ...
      queuedSuccess: 'Ваш ритуал сохранён в локальную очередь устройства (Демо-режим). Когда онлайн-отправка появится, заявка сможет синхронизироваться автоматически.',
    },
    ritualOptions: [
      { id: 'glow', label: 'Свечение', desc: 'Свечение' },
      { id: 'calm', label: 'Покой', desc: 'Покой' },
      { id: 'pulse', label: 'Пульс', desc: 'Ритм' },
    ],
  },
})
...
Object.assign(copy.ru, {
  ...
  privacyCenter: {
    ...
    closeLabel: 'Закрыть центр приватности',
    ...
    consentSwitches: 'Переключатели согласия',
    functionalTitle: 'Функциональная память',
    functionalDescription: 'Сохраняет ритуал и персональное состояние между визитами.',
    analyticsTitle: 'Аналитика',
    analyticsDescription: 'Подключает бережную аналитику только после явного согласия.',
    ...
    notes: [
      'Необходимое хранение используется для сохранения состояния согласия и локальной очереди заявок, если онлайн-отправка недоступна.',
      'Если онлайн-отправка включена, очередь синхронизируется автоматически при следующем успешном соединении.',
      'Аналитика остаётся выключенной, если браузер отправляет сигнал Global Privacy Control.',
    ],
  },
})
...
Object.assign(copy.en, {
  ritualConfigurator: {
    ...
    advisorButton: 'Generate personal result',
    advisorHint: 'After choosing a ritual, you can launch personal generation right away. If remote AI is unavailable, the site will assemble a local result on its own.',
    rituals: [
  ...
  emailSignup: {
    ...
    feedback: {
      ...
      queuedSuccess: 'Your ritual was saved to the device queue (Demo mode). When the live relay becomes available, the request can sync automatically.',
    },
    ritualOptions: [
