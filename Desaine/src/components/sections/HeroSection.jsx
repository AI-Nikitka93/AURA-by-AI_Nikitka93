    ...
    <section id="ritual" className="relative overflow-hidden pt-8 sm:pt-10 lg:pt-12">
      <div className="section-shell grid items-center gap-4 pb-8 sm:gap-6 lg:grid-cols-2 lg:pb-10">
        <div className="relative">
          ...
          <div className="mb-3 inline-flex animate-fade-in items-center rounded-full border border-white/10 bg-white/[0.06] px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-text shadow-insetglass backdrop-blur-glass sm:mb-4 sm:px-4 sm:text-sm">
            {heroContent.portfolioBadge}
          </div>
          <p className="mb-2 animate-fade-in text-xs font-semibold uppercase tracking-[0.16em] text-secondary sm:mb-3 sm:text-sm" style={{ animationDelay: '150ms' }}>{heroContent.eyebrow}</p>
          ...
          <p className="mt-3 max-w-2xl animate-fade-in-up text-sm leading-7 text-text-soft sm:mt-4 sm:text-base" style={{ animationDelay: '200ms' }}>{heroContent.subtitle}</p>
          ...
          <div className="mt-5 flex animate-fade-in-up flex-col gap-2 sm:mt-6 sm:flex-row sm:items-center sm:gap-3" style={{ animationDelay: '300ms' }}>
            <a href={heroContent.primaryCta.href} className="primary-button w-full px-5 py-3 text-xs uppercase tracking-[0.14em] sm:w-auto sm:px-6 sm:py-3.5 sm:text-sm sm:tracking-[0.18em]">
              {heroContent.primaryCta.label}
            </a>
            <a href={heroContent.secondaryCta.href} target="_blank" rel="noreferrer" className="ghost-button w-full px-5 py-3 text-xs uppercase tracking-[0.14em] sm:w-auto sm:px-6 sm:py-3.5 sm:text-sm sm:tracking-[0.18em]">
              {heroContent.secondaryCta.label}
            </a>
          </div>

          <p className="mt-3 max-w-2xl animate-fade-in-up text-sm leading-6 text-text-soft sm:mt-4" style={{ animationDelay: '400ms' }}>
          ...
          <div className="mt-5 grid gap-2 sm:mt-6 sm:grid-cols-3 sm:gap-3">
            {heroHighlights.map((item, index) => (
              <div key={item.label} className="liquid-glass animate-scale-in rounded-xl p-3" style={{ animationDelay: `${500 + index * 100}ms` }}>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-secondary sm:text-sm">{item.label}</p>
                <p className="mt-1 text-sm font-semibold text-text sm:text-base">{item.value}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative reveal-scale">
          ...
          <div className="liquid-panel overflow-hidden p-2 sm:p-3">
            <div className="relative overflow-hidden rounded-xl border border-white/10 sm:rounded-[20px]">
              ...
              <div className="absolute bottom-2 left-2 right-2 flex flex-wrap items-center gap-2 rounded-xl border border-secondary/30 bg-black/45 px-2.5 py-2 backdrop-blur-xl sm:bottom-3 sm:left-3 sm:right-auto sm:gap-2 sm:rounded-full sm:px-3.5 sm:py-2.5">
                <span className="h-2 w-2 animate-glow-pulse rounded-full bg-secondary sm:h-2.5 sm:w-2.5" />
                <span className="text-xs font-semibold uppercase tracking-[0.14em] text-secondary sm:text-sm">{heroContent.signalLabel}</span>
              </div>
            </div>
          </div>
        </div>
